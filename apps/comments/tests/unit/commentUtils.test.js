...
describe('Comment Utils - Unit Tests', () => {
  ...
  describe('textValidation', () => {
    ...
    it('should reject empty text', () => {
      ...
      expect(result.errors.some(e => e.message.includes('text is required'))).toBe(true);
    });
    ...
    it('should reject text that is too short', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toHaveLength(2);
      expect(result.errors.some(e => e.message.includes('text must be at least 1 characters long'))).toBe(true);
    });
    ...
    it('should reject text that is too long', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toHaveLength(1);
      expect(result.errors[0].message).toContain('text must be no more than 250 characters long');
    });
    ...
    it('should handle null and undefined text', () => {
      ...
      expect(nullResult.errors[0].message).toContain('text is required');
      expect(undefinedResult.errors[0].message).toContain('text is required');
    });
  });
  ...
  describe('filterComments', () => {
    ...
    it('should filter comments by parent ID', () => {
      ...
      const result = commentUtils.filterComments(comments, 'parentId', 'parent1');
